...
### 쿼리를 가독성있게 작성해야 하는 이유

- 다른 사람의 쿼리를 봐야하는 경우 & 내 쿼리를 다른 사람이 보는 경우 존재

👉 **나의 시간을 아끼기 위해 효율적으로 가독성있게 쿼리를 작성해야함**
...
3. Alias로 별칭을 지을 때 명시적 vs 암시적인 이름
	보통 명시적으로 사용
...
파티션 필터로 WHERE조건 걸기(오류 발생 가능성 존재)
...
1. 문제 정의 확인 - 요청 사항도 구체적으로 확인

2. Input/Output - 데이터의 Input과 Output 구체적으로 작성해보기
	- 중간결과도 생각해보기
...
Q. 여러분은 포켓몬 트레이너들의 배틀 성적을 분석하는 작업을 맡게 되었습니다. 각 트레이너가 진행한 배틀의 승리 비율을 계산해야 하며, 배틀에 참여한 횟수가 9회 이상인 경우만 계산합니다. 
