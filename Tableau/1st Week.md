...
### 관계
테이블을 물리적으로 합치지 않고 논리적으로 연결하는 방식

### 조인
테이블을 물리적으로 하나의 테이블로 합치는 것
...
숫자(#) : 매출, 수량, 나이 처럼 더하기/빼기 계산을 할 수 있는 값 (정수, 실수)

Abc(문자열) : 가구, 서울, 홍길동 처럼 계산이 안 되는 그냥 글자

📅 (날짜) : 2021-02-08 처럼 연/월/일이 있는 시간 데이터

T|F (BOOL) : True(참) / False(거짓)

🌏 (지구본) : 국가, 도시 처럼 지도 위에 점을 찍을 수 있는 지리 정보
...
- 분석하기 전에 미리 하나의 큰 표로 만들어버린다.

문제 : 조인은 1:N 관계에서 데이터가 중복되는 현상 발생

관계 : 논리적 결합

- 원본 테이블은 그대로 두고, 필요할 때마다 알아서 가져온다.

| **구분** | **조인**  | **관계**  |
| --- | --- | --- |
| **결합 시점** | 분석 전 (데이터 원본 단계) | 분석 중 (쿼리 단계) |
| **결과물 형태** | 하나의 납작한(Flat) 단일 테이블 | 여러 테이블이 독립적으로 존재  |
| **집계 방식** | 행 단위로 결합 후 집계 | 각 테이블을 먼저 집계하고 결과를 결합 |
| **데이터 중복** | 1:N 관계에서 데이터 중복 발생 (뻥튀기) | 중복 없음 (원본 수준 유지) |
| **NULL 값** | 조인 유형(Inner, Left 등)에 따라 데이터 유실 가능 | 데이터 유실 없음 (모든 데이터 유지) |
...
- **관계 :** 옆으로 붙일 때 (가장 추천! ⭐).
- **조인 :** 관계가 안 될 때 쓰는 옛날 방식.
- **혼합 :** 서로 다른 데이터베이스(Ex. 엑셀 + 오라클)를 쓰거나, **집계 수준이 달라서** 그냥 붙이기 힘들 때 **임시로 빌려 쓰는 방식**.

| **구분** | **유니온 (Union)** | **조인 (Join)** | **관계 (Relationship)** | **혼합 (Blending)** |
| --- | --- | --- | --- | --- |
| **방향** | **세로 (행 추가)** ↕️ | **가로 (열 추가)** ↔️ | **가로 (열 추가)** ↔️ | **가로 (정보만 빌려옴)** ↔️ |
| **비유** | 블록 쌓기 | 강력 접착제 | 스마트폰 연결 | 컨닝 페이퍼 |
| **합치는 곳** | 데이터 원본 | 데이터 원본 | 데이터 원본 | **워크시트 (화면)** |
| **특징** | 1월+2월+3월 | 물리적 결합 | 논리적 연결 | **집계(요약) 후 결합** |
...
~~~
유니온으로 결합한다. 
와일드카드 : *2024* or CARD_SUBWAY_MONTH_2024*
~~~
