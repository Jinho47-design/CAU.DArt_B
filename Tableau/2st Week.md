...
> 차원은 대부분 불연속형이며 표 형태로 시각화를 진행했을 때 머리글로 표시가 됩니다. 

> 측정값은 대부분 연속형이고 표 형태로 시각화를 진행했을 때 패널로 표시가 됩니다. 
...
![alt text](<Images_2/누적 병렬 막대그래프.png>)
...
Latitude - 위도
```
![alt text](Images_2/맵-수출,수익.png)

> 경도와 위도 좌표가 있는 위치 데이터를 가지고 있으면 태블로의 지리적 역할을 통해 간단히 맵을 그릴 수 있다. 
